...
public class CBRReuseImpl extends CBRComponent implements CBRReuse{
	...
	public Component reuse(Component simComponent, Component newComponent){
		...
		if (foundIndex > -1) {
			newComponentName = newComponentName.substring(0, foundIndex)
					+ newComponentName.substring(foundIndex	+ simComponentName.length());
		}
		
		// search for keywords within the title and modify the recipe accordingly
		searchComponentDoubled(newCoffeeRecipe, simComponentName, newComponentName);
	...
	void searchComponentDoubled(CoffeeRecipe newRecipe,
		...
		int indexAfter = newComponentName.indexOf(searchStr) + searchStr.length();
	...
	void searchSweetCoffee(CoffeeRecipe newRecipe,
			String simComponentName, String newComponentName) {
		...
		if(componentNameContains(newComponentName, Constants.SWEET)) {
			...
			Ingredient sugar = (Ingredient)newRecipe.getComponentByName("sugar");
			if(sugar != null) {
			...
			// Add sugar
			Ingredient ingredient = new Ingredient("Sugar", Constants.IDEAL_SUGAR_QUANT, Unit.OUNCES);
			newRecipe.addComponent(ingredient);			
			newRecipe.getSteps().add(new AddComponentStep(ingredient));
		}
	}

	void searchDryCoffee(CoffeeRecipe newRecipe,
			String simComponentName, String newComponentName) {
		...
		if(componentNameContains(newComponentName, Constants.DRY)) {
			...
			// Add sugar
			Ingredient ingredient = new Ingredient("Foam", Constants.IDEAL_FOAM_QUANT, Unit.MILLILITERS);
			newRecipe.addComponent(ingredient);			
			newRecipe.getSteps().add(new AddComponentStep(ingredient));
		}
	}

	void searchWetCoffee(CoffeeRecipe newRecipe,
			String simComponentName, String newComponentName) {
		...
		if(componentNameContains(newComponentName, Constants.WET)) {
			...
			// Add sugar
			Ingredient ingredient = new Ingredient("Milk", Constants.IDEAL_MILK_QUANT, Unit.MILLILITERS);
			newRecipe.addComponent(ingredient);			
			newRecipe.getSteps().add(new AddComponentStep(ingredient));
		}
	}
